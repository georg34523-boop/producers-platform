...
export async function listProjects(): Promise<ProjectWithPeople[]> {
  ...
  return (data ?? []) as ProjectWithPeople[]
}

export async function getProject(id: string): Promise<ProjectWithPeople | null> {
  const supabase = await createClient()
  const { data, error } = await supabase
    .from('projects')
  ...
  if (error) throw new Error(error.message)
  return (data as ProjectWithPeople | null) ?? null
}
